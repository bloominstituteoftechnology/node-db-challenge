...
1. Explain the difference between `RDBMS` and `SQL`.
   Ans:-RDBMS is where data is stored in table which contain rows and columns, and these are often related to each other. Where as SQL is a programming language used to storing data, manipulating data, and retrieving data from the databases.

1. Why do tables need a `primary key`?
   Ans:- Primary key allows us to identify the data records uniquely. There may several data entires with same qualities and quantities. primary key will eliminate the confusion when manipulating and retrieving the data.

1. What is the name given to a table column that references the primary key on another table.
   Ans:- A Foreign key
  
1. What do we need in order to have a _many to many_ relationship between two tables.
  Ans: We need a join table that holds foreign keys which references primary keys on these two tables.
