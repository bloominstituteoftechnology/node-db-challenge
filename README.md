...
1. Explain the difference between `RDBMS` and `SQL`.  
   A Relational Database Management System is software that allows us to create a database with data in tables that is associated with other data in different database tables. Structured Query Language is the programming language we use to communicate with database management systems.
2. Why do tables need a `primary key`?  
   A primary key allows us to identify each record in a database table as unique. Primary Keys are needed when we want to join tables together using Foreign Keys that refer to them.
3. What is the name given to a table column that references the primary key on another table.  
   Foreign Key
4. What do we need in order to have a _many to many_ relationship between two tables.  
   Many to many relationship exists when multiple records in multiple tables are related to each other and can be joined using a third table with Foreign Keys.
