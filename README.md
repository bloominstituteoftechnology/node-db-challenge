...
1. Explain the difference between `RDBMS` and `SQL`. RDBMS is a relational database for storing data. SQL is a language for a database.

1. Why do tables need a `primary key`?
So they can have a unique identifier in order to easily identify a single entry.

1. What is the name given to a table column that references the primary key on another table.
Foreign key
1. What do we need in order to have a _many to many_ relationship between two tables.
You would need a third table to join the other two tables together
