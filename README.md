...
1. Explain the difference between `RDBMS` and `SQL`.
  ...
  RDBMS is a term that stands for Relational Data Base Management System. SQL is just one of those systems. 

2. Why do tables need a `primary key`?

  its not necessary, but its a great way to ensure you have a unique identifier and current best practices state that you should have one for each table 
