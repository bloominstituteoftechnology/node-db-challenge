...
1. Explain the difference between `RDBMS` and `SQL`. RDBMS refers to management for databases SQL is a language
2. Why do tables need a `primary key`? For unique ids
3. What is the name given to a table column that references the primary key on another table. Foreign key
4. What do we need in order to have a _many to many_ relationship between two tables. Not Sure
