...
1. Explain the difference between `RDBMS` and `SQL`.
  ~Relational Database Management System stores all of the databses and Structured Query Language is the programing language used to interface with the database
1. Why do tables need a `primary key`?
  ~Primary keys give each entry in a tavlve its own unique value that can be used to find that specific entry
1. What is the name given to a table column that references the primary key on another table.
  ~foreign key
1. What do we need in order to have a _many to many_ relationship between two tables.
  you will need a third table that will have foreign keys referencing the other tables. 
