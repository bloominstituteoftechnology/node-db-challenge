    ...
    .then(project => {
      ...
      db("actions")
        ...
        .catch(err => {
          ...
          res
            .status(500)
            .json({ error: `An error occured retrieving project ${id}.` })
        })
    })
})
...
server.post("/api/projects", (req, res) => {
  ...
  if (!project.name) {
    res.status(400).json({ error: `Project should have a name.` })
  }
  ...
  db("projects")
    ...
    .then(reply => {
      if (!reply || reply.length < 1) {
        res.status(404).json({ error: `No project found with the ID ${id}.` })
      } else {
